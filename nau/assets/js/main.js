...
document.addEventListener("DOMContentLoaded", function (domLoadedEvent) {
	...
	for (let select of document.querySelectorAll('.select')) {
		...
		let opener = select.querySelector('.select__opener');
		let valueCont = select.querySelector('.select__value');
		...
		for (let opt of select.querySelectorAll('.select__item')) {
			opt.onclick = e => {
				let text = opt.textContent;
				input.value = text;
				valueCont.textContent = text;
				closeSelect(select);
	...
	document.onclick = function (e) {
		...
		if (openedSelect !== null &&
			...
			!target.closest('.select__opener') &&
			!target.closest('.select__list')) {
			closeSelect(openedSelect);
		}
	}
	...
	let addBtn = document.querySelectorAll('.learn-best__add-field');
	let learnBestForm = document.querySelector('.learn-best__form');

	for (let i = 0; i < addBtn.length; i++) {
		addBtn[i].onclick = e => {
			e.preventDefault();

			learnBestForm.classList.add('learn-best__form_new-field');
		}
	}
	...
	//our values
	let ourValuesBtns = document.querySelectorAll('.our-values__button');
	let ourBaluesType = document.querySelector('.our-values__type');
	let ourBaluesText = document.querySelector('.our-values__text');

	for (let i = 0; i < ourValuesBtns.length; i++) {
		const btn = ourValuesBtns[i];

		btn.onclick = e => {
			let dataType = btn.dataset.type;
			ourBaluesType.textContent = dataType;

			for (let j = 0; j < ourValuesBtns.length; j++) {
				ourValuesBtns[j].classList.remove('our-values__button_active');
			}
			btn.classList.add('our-values__button_active');
		}
	}

	//faq
	let faqTabs = document.querySelector('.faq__list');
	let faqActiveTab = null;
	...
	if (faqTabs) {
		faqTabs.addEventListener("click", e => {
			...
			if (item) {
				if (faqActiveTab != null) {
					faqActiveTab.classList.remove('faq__item_active');
				}
	...
	// let winnersSlider = document.querySelector('.winners__list');
	// let mousePos;
	// let toutchPos;
	// let elWidth;
	// let elPos = 0;

	const winnersSlider = new core_class('.winners', {
		slidesPerView: "auto",
		freeMode: true,
		// mousewheel: true,
		wrapperClass: 'winners__list',
		slideClass: 'winners__item',
	});


	//browse slider
	const browseSlider = new core_class('.browse-slider', {
		slidesPerView: 'auto',
		// spaceBetween: 24,
		wrapperClass: 'browse-slider__list',
		slideClass: 'browse-slider__item',
		pagination: {
			el: ".pagination",
		...
		navigation: {
			nextEl: ".arrows__item_next",
			prevEl: ".arrows__item_prev",
			disabledClass: 'arrows__item_disabled',
		},
		freeMode: true,
	});

	//product gallery
	const prodGallery = new core_class('.product-gallery', {
		slidesPerView: 1,
		wrapperClass: 'product-gallery__list',
	...
	let menuBlackout = document.querySelector('.collapse-menu__blackout');
	...
	const mediaQueryLarge = window.matchMedia('(max-width: 991px)');
	let header = document.querySelector('.header');
	let headerNav = document.querySelector('.header__nav');
	let headerCollapseMenu = document.querySelector('.collapse-menu__content');

	function handleHeaderChange(e) {
		if (e.matches) {
			headerCollapseMenu.prepend(headerNav);
		} else {
			header.prepend(headerNav);
		}
	}
