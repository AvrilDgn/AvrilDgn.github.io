...
document.addEventListener("DOMContentLoaded", function (domLoadedEvent) {
	...
	window.addEventListener('scroll', highlightHandker);
	...
	for (let i = 0; i < anchorLinks.length; i++) {
		anchorLinks[i].addEventListener('click', function (e) {
			...
			let target = blockID ? document.querySelector(blockID) : document.body;
	...
	const resultsSlider = new core_class('.results__slider .slider__wrapper', {
		// spaceBetween: 30,
	...
	const reviewsSlider = new core_class('.reviews__slider .slider__wrapper', {
		spaceBetween: 30,
		wrapperClass: 'slider__list',
		slideClass: 'slider__item',
		slidesPerView: 1,
	...
	let reviewsSlides = document.querySelectorAll('.reviews__slider .slider__item');
	let fullscreenSLiderClose = document.querySelectorAll('.slider__close-btn');
	let isFS = false;

	for (let i = 0; i < reviewsSlides.length; ++i) {

		reviewsSlides[i].addEventListener('click', function (e) {

			if (isFS) {
				return;
			}

			this.parentNode.parentNode.parentNode.classList.add('slider--fullscreen');
			isFS = true;

			// let realIndexSlider = Number(document.querySelector('.reviews__slider .swiper-slide-active').getAttribute('data-swiper-slide-index'));

			// reviewsSlider.activeIndex = index;
			reviewsSlider.params.slidesPerView = 1;


			for (let itemListPos = 0; itemListPos < reviewsSlides.length; itemListPos++) {
				if (reviewsSlides[itemListPos] === this) {

					reviewsSlider.activeIndex = itemListPos;
					// reviewsSlider.slideTo(itemListPos, false, false);
	...
	for (let i = 0; i < fullscreenSLiderClose.length; ++i) {

		fullscreenSLiderClose[i].addEventListener('click', function (e) {

			e.preventDefault();

			let fullScreenElements = document.querySelectorAll('.slider--fullscreen');
			let activeSlide;

			for (let j = 0; j < fullScreenElements.length; ++j) {
				fullScreenElements[j].classList.remove('slider--fullscreen');
				isFS = false;

				for (let itemListPos = 0; itemListPos < reviewsSlides.length; itemListPos++) {
					if (reviewsSlides[itemListPos].classList.contains('swiper-slide-active')) {

						activeSlide = reviewsSlides[itemListPos];
						// reviewsSlider.slideTo(itemListPos, false, false);
				...
				for (let itemListPos = 0; itemListPos < reviewsSlides.length; itemListPos++) {
					if (reviewsSlides[itemListPos] === activeSlide) {
	...
	let aboutValues = document.querySelectorAll('.about__value');

	for (let i = 0; i < aboutValues.length; i++) {
		let animated;

		if (aboutValues[i].getBoundingClientRect().top >= 0 && aboutValues[i].getBoundingClientRect().top - window.innerHeight <= 0) {
			dist_default()(aboutValues[i], {
				duration: 1000,
				delay: 16,
			});
	...
	setInterval(() => {
		whatsappBtn.classList.toggle('whatsapp-button--animate');
	}, 2500);
