...
document.addEventListener("DOMContentLoaded", function (domLoadedEvent) {
	...
	let menuOpenBtn = header.querySelector('.header__submenu-open');
	let menuCloseBtn = header.querySelector('.header__submenu-close');
	let menuCollapse = header.querySelector('.header__collapse');
	...
	let smMedia = window.matchMedia("screen and (max-width: 768px)");
	let menuBottomContainer = header.querySelector('.header__bottom .container');
	let navigation = header.querySelector('.header__nav');
	let isSm = false;

	if (smMedia.matches) {
		isSm = true;
		menuCollapse.prepend(navigation);
	}
	...
	window.onresize = function (e) {
		...
		if (!isSm && smMedia.matches) {
			isSm = true;
			menuCollapse.prepend(navigation);
		} else if (isSm && !smMedia.matches) {
			isSm = false;
			menuBottomContainer.prepend(navigation);
		}
	}
	...
	for (let i = 0; i < anchorLinks.length; i++) {
		let linkHash = anchorLinks[i].hash;
		...
		anchorLinks[i].addEventListener('click', function (e) {
			...
			let target = document.querySelector(linkHash);
	...
	// 			modalId = openBtn.dataset.target,
