...
function header() {
  ...
  document.addEventListener("click", function(e) {
    if (e.target.closest(".header__menu-btn")) {
      if (!header2.classList.contains("header--menu-opened")) {
        openMenu();
      } else {
        closeMenu();
      }
    } else if (header2.classList.contains("header--menu-opened") && !e.target.closest(".header")) {
      closeMenu();
    }
  });
}
